...
//res对象，我们一般不从里面取信息，而是通过它来定制我们向浏览器输出的信息，比如和header信息，比如想要想浏览器输出的内容。这里我们调用了它们的#send方法，向浏览器输出一个字符串。
app.get("/", function (req,res) {
    res.send('Hello World');
});

//定义好我们app的行为之后，让它监听本地的3000端口。这里的第二个函数是个回调函数，会在listen动作成功后执行，我们这里执行了一个命令输出操作，告诉我们监听动作已完成。
